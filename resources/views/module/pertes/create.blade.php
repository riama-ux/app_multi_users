...
<div class="card card-preview">
    <div class="card-inner">
        ...
        <form action="{{ route('module.pertes.store') }}" method="POST">
            ...
            @php
                $magasin = \App\Models\Magasin::find(session('magasin_id'));
            @endphp

            {{-- Affichage magasin sélectionné (lecture seule) --}}
            <div class="mb-3">
                <label class="form-label">Magasin concerné</label>
                <input type="text" class="form-control" value="{{ $magasin ? $magasin->nom : 'Aucun magasin sélectionné' }}" disabled>
            </div>
